...
# what the locale is of another category.  (As an example, this test showed
# that some platforms require LC_CTYPE to be the same as LC_COLLATION, and/or
# LC_MESSAGES for proper results, so that Perl had to change to bring these
# into congruence under the hood).  And it also catches where there is
# interference between multiple threads.
...
sub add_trials($$;$)
{
    ...
    my $locale_pattern = shift // "";   # If defined, the test will be created
                                        # only for locales that match this
