...
#    ifndef USE_PL_CURLOCALES
    ...
    DEBUG_Lv(PerlIO_printf(Perl_debug_log,
             "(%d): emulate_setlocale_i now using %p\n", line, new_obj));
...
#    endif
    ...
    my_langinfo_c(RADIXCHAR, LC_NUMERIC, NULL, &radix, NULL, &utf8ness);
...
#    ifdef HAS_SOME_LANGINFO

            {
                ...
                Perl_sv_catpvf(aTHX_ PL_warn_locale, "; codeset=%s",
                               my_langinfo_c(CODESET, LC_CTYPE, NULL,
...
{
    HV * retval;
    int numeric_locale_is_utf8  = UTF8NESS_UNKNOWN;
    int monetary_locale_is_utf8 = UTF8NESS_UNKNOWN;
    HV * (*copy_localeconv)(pTHX_ const struct lconv *, int, int);
...
#  ifndef HAS_SOME_LANGINFO
        ...
        /* To avoid some extra unused parameters, we use the
         * 'numeric_locale_is_utf8' parameter to instead mean 'item' in the
         * call to this function; they both have the same type.  Hopefully,
         * this isn't too confusing; it allows for not having to have some
         * preprocessor-controlled function declarations. */
        numeric_locale_is_utf8 = item;
    }
...
#  endif
        ...
        assert(locale_is_utf8 != UTF8NESS_UNKNOWN);
        ...
        while (strings->name) {
            ...
            if (value && *value) {
                bool is_utf8 =  /* Only make UTF-8 if required to */
                    (2 == (get_locale_string_utf8ness_i(locale,
                                                        cat_index,
                                                        value,
                                                        locale_is_utf8)));
                                    ...
                                    const int unused)
{
     ...
     * locale.  This logic exists for other reasons, and by doing it this way,
                ...
                int * utf8ness)
{
    ...
    DEBUG_Lv(PerlIO_printf(Perl_debug_log,
            ...
            if (locale == NULL) {
                ...
                PerlIO_printf(Perl_debug_log, "locale %s\n", locale);
            });
/*--------------------------------------------------------------------------*/
 ...
 * Configuration */
    ...
    /* If the desired locale to get the information about isn't the current
     * one, switch to it, and call ourselves recursively */
    if (locale == NULL) {
        locale = querylocale_i(cat_index);
    }
...
#    if  defined(HAS_SNPRINTF)                                               \
     && (defined(TS_W32_BROKEN_LOCALECONV) || ! defined(HAS_SOME_LOCALECONV))
         ...
         * issues. */
        {
            ...
            /* Here, the syntax of the number wasn't like we expected.  If we
             * do have localeconv available, drop down to try that */

#      ifndef HAS_SOME_LOCALECONV
...
#    endif
      ...
      case MON_9: case MON_10: case MON_11: case MON_12:

        {
             ...
             * time, it all works */
            temp = my_strftime8(format, 30, 30, hour, mday, mon,
                                2011, 0, 0, 0, utf8ness);
            ...
            if (! return_format) {
                return retval;  /* *utf8ness was set by my_strftime8() */
            }
                         ...
                         int * utf8ness)
{
    ...
    DEBUG_Lv(PerlIO_printf(Perl_debug_log, "fmt=%s, retval=%s", fmt, retval);
...
S_toggle_locale_i(pTHX_ const unsigned cat_index, const char * new_locale)
{
    ...
    DEBUG_Lv(PerlIO_printf(Perl_debug_log,
       "toggle_locale_i: index=%d(%s), wanted=%s, actual=%s\n",
       cat_index, category_names[cat_index], new_locale, locale_to_restore_to));
...
#  else
    ...
    DEBUG_Lv(PerlIO_printf(Perl_debug_log, "found codeset=%s, is_utf8=%d\n",
                                           codeset, retval));
 ...
 * locale', it uses the text from the C locale.  If Perl is compiled to
 ...
 * would greatly obscure the logic of the implementation various
 * implementations, the whole function is repeated for each configuration, with
 * some common macros. */
